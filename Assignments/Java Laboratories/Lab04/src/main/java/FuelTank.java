...
public class FuelTank {
    ...
    public float fill(float amount) {
        if (this. capacity - this.level*this.capacity < amount) this.level += (amount/capacity);
        System.out.println("The capacity is: "+this.capacity+" "+"and the level is" + this.level);
    ...
    public boolean releaseFuel(float amount) {
        if (amount < this.getCapacity()){
            if (this.getLevel() - amount/capacity > 0) this.setLevel(amount/this.capacity);
        }
    ...
    public float setLevel(float amount){
        this.level = amount;
        return level;
    }
